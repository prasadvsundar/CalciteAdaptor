...
/**
 * Table based on a EXCEL file that can implement simple filtering.
 *
 * <p>It implements the {@link FilterableTable} interface, so Calcite gets
 * data by calling the {@link #scan(DataContext, List)} method.
 */
public class ExcelFilterableTable extends ExcelTable
    implements FilterableTable {
  /** Creates a EXCELFilterableTable. */
  ExcelFilterableTable(File file, RelProtoDataType protoRowType) {
    super(file, protoRowType);
  }

  ExcelFilterableTable(File file, RelProtoDataType protoRowType, String sheet) {
	    super(file, protoRowType, sheet);
	  }
  ...
  public Enumerable<Object[]> scan(DataContext root, List<RexNode> filters) {
    ...
    final int[] fields = ExcelEnumerator.identityList(fieldTypes.size());
    final AtomicBoolean cancelFlag = DataContext.Variable.CANCEL_FLAG.get(root);
    return new AbstractEnumerable<Object[]>() {
      public Enumerator<Object[]> enumerator() {
        return new ExcelEnumerator<>(file, cancelFlag, false, filterValues,
            new ExcelEnumerator.ArrayRowConverter(fieldTypes, fields));
      }
    };
  }
